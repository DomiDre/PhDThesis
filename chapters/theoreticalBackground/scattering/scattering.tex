...
\end{align}
where $\vec{\mu}_n$ is the magnetic moment of the neutron, which has a magnitude of $\mu_n \eq 9.662 \cdot 10^{-27} \, \unit{JT^{-1}}$.
The quantum mechanical nature of the spin is ignored for brevity at the moment for the following discussion.
The potential tries to align the neutron magnetic moment to the magnetic field to minimize the energy.
Additionally, the magnetic field also leads to a precession of $\vec{\mu}_n$ around $\vec{B}$ according to the Landau-Lifshitz equation
\begin{align}
  \dot{\vec{\mu}}_n \eq \gamma  \vec{\mu}_n \times \vec{B},
\end{align}
which can lead to a flipping of the neutron spin, from a classical point of view, when the polarization of the neutron is not aligned with the magnetic field. Here, $\gamma$ is the gyromagnetic ratio of the neutron $\gamma \eq -1.83 \cdot 10^8 \, \unit{T^{-1} s^{-1}} $

A magnetic sample generates a field for once by the orbital motion of the electrons $\vec{B}_L$, given by the Biot-Savart law, and by the electron spins $\vec{B}_S$ which can be modelled for a single electron at position $\vec{r}_i$ by a dipole field
\begin{align}
...
\end{align}
with $\mu_B \approx 9.274 \cdot 10^{-24} \, \unit{JT^{-1}}$ (ignoring the anomalous magnetic dipole moment of the electron), to write the integral over the potential in \refeq{eq:theoreticalBackground:scattering:scatteringTheory:differentialCrossSectionIntegralOverPotential} for the spin-dependant interaction as
\begin{align}
...
\end{align}
To include the quantum mechanical nature of the neutron spin, one has to note that the three components of the neutron magnetic moment are actually given in terms of the spin operator
\begin{align}
...
\end{align}
which can be represented by the three Pauli matrices.
...
This subtle difference allows through careful analysis of the four scattering channels to measure the direction of the magnetization in the plane perpendicular to the scattering vector.
