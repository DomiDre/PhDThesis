...
\begin{document}
  \subsection{Reflectometry}
    ...
    Using that the energy is given by the wave vector of the incoming neutron $k_{z,\,\mathrm{in}}$ outside the material for elastic scattering, the Schr\"odinger equation writes as an equation that determines the wave vector of the neutron inside the sample at height $z$
    \begin{align}
      k_z^2 \eq   k_{z,\,\mathrm{in}}^2 - 4 \pi  \mathrm{SLD}(z).
    \end{align}
    $k_{z,\,\mathrm{in}}$ relates to the scattering vector magnitude in reflectometry geometry by the simple relation $q \eq 2 k_{z,\,\mathrm{in}}$.
