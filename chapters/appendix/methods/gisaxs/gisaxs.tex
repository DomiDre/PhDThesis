...
\begin{document}
  \section{Grazing-Incidence Small-Angle X-Ray Scattering (GISAXS)}
    ...
    In this work, grazing-incidence small-angle X-ray scattering has been performed for multiple samples the GALAXI instrument (\refapp{ch:appendix:lss:galaxi}).
    For the experiment, the sample is placed on a flat holder as shown in \reffig{appendix_methods_gisaxs_sample}, that can be moved in vertical direction to center the sample and in the plane to set the beam to the center of the sample.
    ...
    Doing the same procedure with AgBH as described in \refapp{app:methods:saxs}, the sample-to-detector distance $L_\mathrm{SDD}$ and the beam center position $(y_c,\,z_c)$ are determined.
    Using these, for each pixel on the detector with $(y,\,z)$ as pixel coordinate, the scattering angles are determined geometrically by comparison with \reffig{fig:appendix:methods:gisaxs:geometry}
    \begin{align}
      \begin{split}
