...
\begin{document}
  \subsection{GALAXI}\label{ch:lss:galaxi}
    ...
    Both can and have been treated by using the public Python packages as described in the respective chapters for SAXS (\refsec{ch:methods:saxs}) and GISAXS(\refsec{ch:methods:gisaxs}).
    ...
    \end{figure}
    To quantify the beam divergence of the instrument, the beam width of the attenuated direct beam can be measured as shown in \reffig{fig:lss:galaxi:directBeam}.
    ...
    This value can be understood by the collimation of the system as presented in \refsec{ch:methods:sans}.
    ...
    Using \refeq{eq:methods:sans:instrumentalResolution} to determine the first two addends contributing to the instrumental resolution, they are evaluated to $\sigma_q^\mathrm{collimation} \approx 0.01 \unit{nm^{-1}}$, which is close to the Gaussian beam width.
