...
\begin{document}
  \section{X-Ray Reflectometry (XRR)}
    ...
    As visible in the right plot of \reffig{fig:methods:xrr:alignment}, a peak is visible from which both $\alpha_i$ and $\alpha_o$ can then be calibrated by the peak position to the expected angle that was chosen.
    ...
    As typically the footprint of the beam on the sample is initially larger than the sample size, no plateau is measured for small angles as expected but the intensity increases until the complete beam hits the sample as shown in \reffig{fig:methods:xrr:footprint}.
