...
\begin{document}
  ...
  \paragraphNewLine{Small-Angle Scattering}
    ...
    Both the nuclear coherent scattering lengths and the atomic form factor are tabulated in \refapp{appendix:slds}.
    ...
    \end{align}
    The volumes of the two phases are determined from the particle size, shell thickness and superball exponent on the fly during the fitting process, such that y is readily determined with varied particle dimensions and cobalt content $x$ in \ch{Co_x Fe_{3-x}O4}.
    ...
    The scattering length density of the oleic acid surfactant shell and the solvents SLD is fixed to the values tabulated in \refapp{appendix:slds}.
  ...
  \paragraphNewLine{Vibrating Sample Magnetometry}
    ...
    \begin{figure}[tb]
      \centering
      \includegraphics{monolayer_PPMS_Dispersion_ZFC_FC_Ac_CoFe_C}
      \includegraphics{monolayer_VSM_DispersionHyst_Ac_CoFe_C}
    ...
    As the temperature-dependent magnetization of the liquid dispersions showed in all cases jumps upon freezing depicted in \reffig{fig:monolayers:charMethods:JumpMagnetization}, the rescaling of the dispersion $10 \unit{K}$ is not straight forward.
