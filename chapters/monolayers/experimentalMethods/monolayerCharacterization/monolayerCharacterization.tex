...
\begin{document}
  ...
  \paragraphNewLine{X-Ray Reflectometry}
    ...
    For footprint correction (see \refsec{ch:methods:xrr}), the beam width is estimated by the size of the collimation slits, which is $0.2 \unit{mm}$, and the samples both have a width of $10 \unit{mm}$.
    ...
    In total this model has $8$ structural parameters: $4$ parameters to describe the thickness of the nanocubes, the two OA and the \ch{SiO2} layers and $4$ roughness parameters for the interfaces \ch{Si}/\ch{SiO2}, \ch{SiO2}/OA, OA/nanocube and OA/air.
