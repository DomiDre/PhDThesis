...
\begin{document}
  ...
  For Ol-CoFe-C, the information of the ratio of cobalt to iron in the nanoparticle is not enough to deduce on the formula unit. Here, the core-shell nanoparticles have two phases that may contain cobalt and iron, namely \ch{Co_x Fe_{3-x} O4} and \ch{Fe_y Co_{1-y} O}
  % As XRD in \refsec{sec:monolayers:nanoparticle:xrd} however suggests that $97 \%$ of the nanoparticle volume is in a wustite phase, the ratio can be used to approximate the relative composition of iron and cobalt in the w\"ustite phase.
  % Thereby the core of Ol-CoFe-C has the composition \ch{Fe_{0.71} Co_{0.29} O}.
  % The shell composition could only be given with a large error margin due to the low volume fraction, therefore it is assumed to be \ch{CoFe2O4} for simplicity.

  The density of the particles is then estimated using the determined composition and the lattice constant from XRD \refsec{sec:monolayers:nanoparticle:xrd}.
