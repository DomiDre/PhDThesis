...
\begin{document}
  ...
  \begin{table}[tb]
    \centering
    \caption{\label{tab:looselyPackedNP:nanoparticle:gisaxs}Parameters for the hard-sphere structure factor in Percus-Yevick approximation shown in \reffig{fig:looselyPackedNP:layer:gisaxsSC_IOS_11} and \reffig{fig:looselyPackedNP:layer:gisaxsSC_IOS_7} for both SC-IOS-11 and SC-IOS-7 respectively. $R_\mathrm{HS}$ is the hard-sphere radius and $\eta$ the packing fraction of the structure factor. Using both values the Wigner-Seitz radius $\bar{r}$ is determined, as well as the average radius of equivalent spheres in a close packing $\bar{r}_\mathrm{CP}$.}
    \begin{tabular}{ c | l | l }
  ...
  Estimating the average particle spacing by the Wigner-Seitz radius $\bar{r}$, which is given by the inverse of the particle number density in \refeq{eq:looselyPackedNP:layers:wignerSeitz}, a mean Wigner-Seitz radius of $7.44 \unit{nm}$ is obtained for SC-IOS-11 and $5.54 \unit{nm}$ for SC-IOS-7.
  ...
  % In conclusion, the GISAXS evaluation is giving information on both, the minimum distance the nanoparticles approach each other via the hard-sphere radius, and the average particle density.
