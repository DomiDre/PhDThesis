...
Both techniques allow to a certain extent to model the average individual nanoparticle magnetization and are compared in the following.
To evaluate the VSM data a Langevin curve with a log-normal distribution $p(x; \mu, \sigma)$ of the magnetic moment and a possible excess susceptibility is fitted as described by
\begin{align}
...
For SANSPOL, a magnetic field of $B\eq 515 \unit{mT}$ is applied perpendicular \& horizontal to the beam direction and the scattered neutrons are counted on a position-sensitive detector for up and down polarized neutrons without polarization analysis after the scattering.
