...
\begin{document}
  \paragraphNewLine{Scanning Electron Microscopy}
    The spin-coated nanospheres on silicon substrate SC-IOS-7 and SC-IOS-11 are qualitatively viewed by SEM micrographs measured with a Neon Zeiss 40 (\refsec{ch:instruments:laboratoryInstruments:sem}).
  ...
  \paragraphNewLine{X-Ray Reflectometry}
    ...
    To evaluate the reflectivity, a footprint correction is performed as described in \refsec{ch:methods:xrr}, assuming an equidistributed beam profile.
    ...
    \begin{figure}[htbp]
      \centering
      \includegraphics{looselyPackedNP_stackOfSpheres}
      \includegraphics{looselyPackedNP_stackOfSpheresCrossSection}
    ...
    In this formulation it is now straight-forward to extend it to overlapping nanosphere layers by summing the respective area fractions $\eta_x$ for each layer before determining the SLD by \refeq{eq:looselyPackedNP:layerCharacterization:sldFromAreaFraction}.
    The path via the area fractions correctly adds the overlapping areas as depicted in \reffig{fig:looselyPackedNP:layerCharacterization:sphereStackProfile} and clearly separate, which part in the lateral averaging comes from the nanoparticle material and the oleic acid surfactant.
