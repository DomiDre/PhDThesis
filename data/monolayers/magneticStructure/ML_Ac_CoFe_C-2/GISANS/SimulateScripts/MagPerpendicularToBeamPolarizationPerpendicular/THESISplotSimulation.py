...
chapter = 'monolayers'
sample_name = 'ParacrystalSimulationMagPerpendicularBeamPolarizationPerpendicular'
...
for ix in range(Nx):
  for iy in range(Ny):
    ...
    ax_config.add_patch(
        patches.Rectangle(
            (ix*pp_distance, iy*pp_distance),   # (x,y)
            cube_size,          # width
            cube_size,          # height
            fill=True,
            color='white'
    ...
    ax_config.arrow(ix*pp_distance + cube_size/2,
              iy*pp_distance + cube_size/4,
