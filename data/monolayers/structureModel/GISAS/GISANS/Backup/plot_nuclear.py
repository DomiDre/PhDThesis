...
if plot2d:
    ...
    pcm = ax.pcolormesh(\
            qy_sat_minus, qz_sat_minus, (data_sat_plus+data_sat_minus).T,\
            norm=mcolors.LogNorm(), cmap=custom_cmap,vmin=vmin, vmax=vmax)

    pcm2 = ax2.pcolormesh(\
            qy_rem_minus, qz_rem_minus, (data_rem_plus+data_rem_minus).T,\
            norm=mcolors.LogNorm(), cmap=custom_cmap, vmin=vmin, vmax=vmax)
    ...
    cbar = plt.colorbar(pcm2, cax=cax)
    ...
    fig.savefig('2dImagesSatRemTotalSum.png')
    plt.show()