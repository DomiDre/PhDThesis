...
def plot_gisas(filepath, sample_name, f, A10, q10, dq10, c10):
  ...
  if dq10 is not None:
    ...
    # p_copy = p_lorentzian.copy()
    # p_copy['mu'].value = np.sqrt(2)*p_copy['mu'].value*0.97
    # p_copy['A'].value = 0.07*p_copy['A'].value
    # p_copy['sig'].value = p_copy['sig'].value*(2)
    # plot_lorentzian = np.logical_and(p_copy['mu'].value-1.2*plot_width < qyslice, qyslice < p_copy['mu'].value+1.2*plot_width)
    # ax2.plot(
    #   qyslice[plot_lorentzian], lorentzian(qyslice[plot_lorentzian], p_copy),
    #   linestyle='-', marker='None', color='black', zorder=10, alpha=0.5
    # )
    # p_copy = p_lorentzian.copy()
    # p_copy['mu'].value = 2*p_copy['mu'].value*1.03
    # p_copy['A'].value = 0.04*p_copy['A'].value
    # p_copy['sig'].value = p_copy['sig'].value*4
    # plot_lorentzian = np.logical_and(p_copy['mu'].value-2*plot_width < qyslice, qyslice < p_copy['mu'].value+2*plot_width)
    # ax2.plot(
    #   qyslice[plot_lorentzian], lorentzian(qyslice[plot_lorentzian], p_copy),
    #   linestyle='-', marker='None', color='black', zorder=10, alpha=0.5
    # )
...
with open('peak_fit_parameters', 'w') as f:
  ...
  plot_gisas(cwd+"/GISAXS/DD175_28.h5", 'ML-Ac-CoFe-C-2', f, 1.28e-4, 0.4726, 5e-2, 2e-7)
