...
from utils import load_xye, gaussian, residuum, residuum_no_error, get_power, lorentzian
import lmfit
import matplotlib.pyplot as plt

savefile = 'appendix_instruments_GALAXIDirectBeamLorentzian'
...
p = lmfit.Parameters()
p.add('A', 1, min=0, vary=True)
p.add('mu', 0.0, vary=True)
p.add('sig', 0.010, min=0, max = 0.3)
p.add('c', 00, min=0, vary=False)

fit_result = lmfit.minimize(residuum_no_error, p, args=(q, I, sI, lorentzian))
...
ax.plot(q_plot, lorentzian(q_plot, p_result), marker='None',\
