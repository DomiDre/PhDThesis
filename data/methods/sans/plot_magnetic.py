...
vmin, vmax = 1e-6, 1e-3
sdd = 8000
beamcenter_y, beamcenter_z = 65.4982, 126.261
pixelsize_y, pixelsize_z = 8, 4
wavelength = 0.721
...
def open_nexus_file(nexus_file):
  ...
  rawdata = f[f"entry0/data/MultiDetector_linear_data"][:,:,0]
  # data = data.T[:,::-1]
...
def plot(filepath, numbers, sample_name):
  ...
  ax.text(0.33, -0.4, r"$\vec{\mathit{B}}$")
  ax.annotate("", xy=(0.45, -0.42), xytext=(0.28, -0.42),
    arrowprops=dict(arrowstyle="->"))
  deg = 90
  delta = 10
  ax.plot(
    [0,np.cos((deg-delta)*np.pi/180)],
    [0,np.sin((deg-delta)*np.pi/180)],
  ...
  ax.plot(
    ...
    color='black', marker='None')
  ax.plot(
    [0,np.cos((180+deg-delta)*np.pi/180)],
    [0,np.sin((180+deg-delta)*np.pi/180)],
    color='black', marker='None')
  ax.plot(
    [0,np.cos((180+deg+delta)*np.pi/180)],
    [0,np.sin((180+deg+delta)*np.pi/180)],
...
plot('./rawdata/213', [434], 'MagneticScattering_RFoff')
plot('./rawdata/213', [435], 'MagneticScattering_RFon')
