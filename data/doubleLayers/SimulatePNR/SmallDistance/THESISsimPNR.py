...
savefile = chapter+'_NR_simPNRsmallGap'
...
ax.plot(q_p_afm, I_p_afm2*1e-1, alpha=0.8, marker='None', label='AFM R+')
ax.plot(q_m_afm, I_m_afm2*1e-1, alpha=0.8, marker='None', label='AFM R-')
...
ax_sld.plot(z_afm2, rhoMag_afm2-0.1, marker='None', ls='-', color='#FAAB2D')
# ax_sld.plot(z_afm2, rhoMag_afm2, marker='None', ls='-')
ax_sld.set_xlabel("$\mathit{z} \,/\,nm$", fontsize=8)
ax_sld.set_ylabel(r"$\rho \, / \, 10^{-6} \AA^{-2}$", fontsize=8)
ax_sld.set_xticks([0, 20, 40])
ax_sld.set_yticks([0, 1, 2, 3])
ax_sld.set_xlim([-10, 55])
